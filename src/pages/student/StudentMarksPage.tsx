        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 relative z-10">
          <div>
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-2xl font-bold text-slate-900 font-serif-institutional tracking-tight">Academic Performance & Marks Scorecard</h1>
          ...
          <div className="px-4 py-2 bg-slate-50 border border-slate-200 rounded-xl text-right shadow-2xs">
            <div className="text-xs text-slate-500">Enrolled Student</div>
            <div className="text-sm font-bold text-slate-900">{currentStudent?.full_name}</div>
            <div className="text-xs font-mono text-slate-700 font-semibold">{currentStudent?.roll_number}</div>
          </div>
        </div>
        ...
        {!isLoading && (scorecard.length > 0 || unpublishedSubjects.length > 0) && (
          <div className="mt-6 pt-4 border-t border-slate-200 grid grid-cols-2 sm:grid-cols-4 gap-3">
            <div className="p-3 rounded-xl bg-slate-50 border border-slate-200/80">
              <span className="text-[10.5px] text-slate-500 block">Published Scorecards</span>
            ...
            <div className="p-3 rounded-xl bg-slate-50 border border-slate-200/80">
              <span className="text-[10.5px] text-slate-500 block">Awaiting Publication</span>
              <span className="text-base font-bold text-amber-700 mt-0.5 block font-mono">
            ...
            <div className="p-3 rounded-xl bg-slate-50 border border-slate-200/80">
              <span className="text-[10.5px] text-slate-500 block">Total Internal Score</span>
              <span className="text-base font-bold text-slate-900 mt-0.5 block font-mono">
                {statsSummary.totalObtained} <span className="text-xs font-normal text-slate-400">/ {statsSummary.totalMax}</span>
              </span>
            </div>
            <div className="p-3 rounded-xl bg-slate-50 border border-slate-200/80">
              <span className="text-[10.5px] text-slate-500 block">Overall Percentage</span>
              <span className="text-base font-bold text-slate-900 mt-0.5 block font-mono">
            ...
            >
              ...
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-4 border-b border-slate-200">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h3 className="text-lg font-bold text-slate-900 font-serif-institutional">{item.subjectName}</h3>
                    <p className="text-xs text-slate-500">Faculty In-charge: {item.facultyName}</p>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div className="text-right pl-4 border-l border-slate-200">
                    <div className="text-[11px] text-slate-500">Internal Score</div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 pt-1">
                ...
                <div className="bg-slate-50 p-4 rounded-xl border border-slate-200/80 space-y-2.5">
                  <h4 className="text-xs font-bold text-slate-700 uppercase tracking-wider flex items-center gap-1.5">
                    <Award className="w-3.5 h-3.5 text-emerald-700 font-bold" /> Sessional Examinations
                  </h4>
                  <div className="space-y-2 text-xs">
                    {item.sessionalMarks.sessionals.length === 0 ? (
                      ...
                      item.sessionalMarks.sessionals.map((s, idx) => (
                        <div key={s.assessmentId || idx} className="flex justify-between items-center py-1 border-b border-slate-200/80">
                  ...
                  <div className="space-y-2 text-xs">
                    {item.quizMarks.length === 0 ? (
                      ...
                      item.quizMarks.map(q => (
                        <div key={q.quizId} className="flex justify-between items-center py-1 border-b border-slate-200/80">
                  ...
                  <div className="space-y-2 text-xs">
                    {item.assignmentMarks.length === 0 ? (
                      ...
                      item.assignmentMarks.map(a => (
                        <div key={a.assignmentId} className="flex justify-between items-center py-1 border-b border-slate-200/80">
                ...
                >
                  <div className="flex items-start justify-between gap-3">
                    <div>
                      ...
                      <h4 className="text-sm font-bold text-slate-900 mt-1.5">{sub.subjectName}</h4>
                  ...
                  <div className="p-3 rounded-xl bg-slate-50 border border-slate-200/60 text-[11.5px] text-slate-600 flex items-center justify-between">
                    <span>Attendance: <strong className={sub.percentage !== null && sub.percentage >= 75 ? 'text-emerald-700 font-bold' : 'text-slate-300'}>{sub.percentage !== null ? `${sub.percentage}%` : 'No data'}</strong></span>
