        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 relative z-10">
          <div>
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-2xl font-bold text-[#172033] font-serif-institutional tracking-tight">Academic Performance & Marks Scorecard</h1>
          ...
          <div className="px-4 py-2 bg-slate-50 border border-slate-200 rounded-xl text-right shadow-2xs">
            <div className="text-xs text-[#52627A] font-semibold">Enrolled Student</div>
            <div className="text-base font-bold text-[#172033]">{currentStudent?.full_name}</div>
            <div className="text-xs font-mono text-[#52627A] font-semibold">{currentStudent?.roll_number}</div>
          </div>
        </div>
        ...
        {!isLoading && (scorecard.length > 0 || unpublishedSubjects.length > 0) && (
          <div className="mt-6 pt-4 border-t border-slate-200 grid grid-cols-2 sm:grid-cols-4 gap-3">
            <div className="p-3.5 rounded-xl bg-slate-50 border border-slate-200/80">
              <span className="text-xs font-semibold uppercase tracking-wider text-[#52627A] block">Published Scorecards</span>
              <span className="text-xl font-bold text-[#172033] mt-1 block font-mono">
                {statsSummary.publishedCount} <span className="text-xs font-normal text-[#52627A]">Subject{statsSummary.publishedCount !== 1 ? 's' : ''}</span>
              </span>
            </div>
            <div className="p-3.5 rounded-xl bg-slate-50 border border-slate-200/80">
              <span className="text-xs font-semibold uppercase tracking-wider text-[#52627A] block">Awaiting Publication</span>
              <span className="text-xl font-bold text-amber-800 mt-1 block font-mono">
                {statsSummary.unpublishedCount} <span className="text-xs font-normal text-[#52627A]">Subject{statsSummary.unpublishedCount !== 1 ? 's' : ''}</span>
              </span>
            </div>
            <div className="p-3.5 rounded-xl bg-slate-50 border border-slate-200/80">
              <span className="text-xs font-semibold uppercase tracking-wider text-[#52627A] block">Total Internal Score</span>
              <span className="text-xl font-bold text-[#172033] mt-1 block font-mono">
                {statsSummary.totalObtained} <span className="text-xs font-normal text-[#52627A]">/ {statsSummary.totalMax}</span>
              </span>
            </div>
            <div className="p-3.5 rounded-xl bg-slate-50 border border-slate-200/80">
              <span className="text-xs font-semibold uppercase tracking-wider text-[#52627A] block">Overall Percentage</span>
              <span className="text-xl font-bold text-[#172033] mt-1 block font-mono">
      ...
      <div className="space-y-5">
        {isLoading ? (
          ...
          <div className="py-16 text-center bg-slate-50 border border-slate-200 rounded-2xl">
            <GraduationCap className="w-12 h-12 text-[#52627A] mx-auto mb-3 opacity-50" />
            <h3 className="text-lg font-bold text-[#172033]">No Assessment Records Yet</h3>
            <p className="text-sm text-[#52627A] max-w-md mx-auto mt-1">
          ...
          scorecard.map(item => (
            ...
            >
              ...
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-4 border-b border-slate-200">
                <div className="flex items-center gap-3">
                  <span className="px-3 py-1 rounded-xl text-xs font-bold bg-slate-100 text-[#172033] border border-slate-200 font-mono">
                    {item.subjectCode}
                  </span>
                  <div>
                    <h3 className="text-lg font-bold text-[#172033] font-serif-institutional">{item.subjectName}</h3>
                    <p className="text-sm text-[#52627A]">Faculty In-charge: {item.facultyName}</p>
                  </div>
                </div>

                <div className="flex items-center gap-4">
                  <div className="text-right">
                    <div className="text-xs font-semibold uppercase tracking-wider text-[#52627A]">Attendance</div>
                    <div className={clsx(
                  ...
                  <div className="text-right pl-4 border-l border-slate-200">
                    <div className="text-xs font-semibold uppercase tracking-wider text-[#52627A]">Internal Score</div>
                ...
                <div className="bg-slate-50 p-4 rounded-xl border border-slate-200/80 space-y-2.5">
                  ...
                  <div className="space-y-2 text-sm">
                    {item.sessionalMarks.sessionals.length === 0 ? (
                      <p className="text-[#52627A] py-3 text-center">No sessional marks published yet.</p>
                    ) : (
                      item.sessionalMarks.sessionals.map((s, idx) => (
                        <div key={s.assessmentId || idx} className="flex justify-between items-center py-1.5 border-b border-slate-200">
                          <span className="text-[#52627A] font-medium truncate max-w-[160px]">{s.title}:</span>
                ...
                <div className="bg-slate-50 p-4 rounded-xl border border-slate-200/80 space-y-2.5">
                  ...
                  <div className="space-y-2 text-sm">
                    {item.quizMarks.length === 0 ? (
                      <p className="text-[#52627A] py-3 text-center">No quizzes conducted.</p>
                    ) : (
                      item.quizMarks.map(q => (
                        <div key={q.quizId} className="flex justify-between items-center py-1.5 border-b border-slate-200">
                          <span className="text-[#52627A] font-medium truncate max-w-[160px]">{q.title}:</span>
                          <span className="font-mono font-bold text-[#172033]">
                ...
                <div className="bg-slate-50 p-4 rounded-xl border border-slate-200/80 space-y-2.5">
                  ...
                  <div className="space-y-2 text-sm">
                    {item.assignmentMarks.length === 0 ? (
                      <p className="text-[#52627A] py-3 text-center">No assignments assigned.</p>
                    ) : (
                      item.assignmentMarks.map(a => (
                        <div key={a.assignmentId} className="flex justify-between items-center py-1.5 border-b border-slate-200">
                          <span className="text-[#52627A] font-medium truncate max-w-[160px]">{a.title}:</span>
                          <span className="font-mono font-bold text-[#172033]">
              ...
              {unpublishedSubjects.map(sub => (
                ...
                >
                  <div className="flex items-start justify-between gap-3">
                    <div>
                      <span className="px-2.5 py-0.5 rounded-lg text-xs font-bold bg-slate-100 text-[#172033] border border-slate-200 font-mono">
                        {sub.subjectCode}
                      </span>
                      <h4 className="text-base font-bold text-[#172033] mt-1.5">{sub.subjectName}</h4>
                      <p className="text-sm text-[#52627A] mt-0.5">Faculty: {sub.facultyName}</p>
                    </div>
                  ...
                  <div className="p-3 rounded-xl bg-slate-50 border border-slate-200 text-xs text-[#52627A] flex items-center justify-between">
                    <span>Attendance: <strong className={sub.percentage !== null && sub.percentage >= 75 ? 'text-emerald-700 font-bold' : 'text-[#172033] font-bold'}>{sub.percentage !== null ? `${sub.percentage}%` : 'No data'}</strong></span>
                    <span className="text-[#52627A] italic">Continuous assessment pending faculty publication</span>
                  </div>
                </div>
              ))}
            </div>
