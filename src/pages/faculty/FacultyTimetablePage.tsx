      ...
      <div className="bg-white rounded-3xl p-6 sm:p-7 border border-slate-200/90 shadow-xs space-y-4">
          ...
          <div className="space-y-1">
            <div className="flex items-center gap-2">
              <span className="px-2.5 py-0.5 rounded-full text-[10px] font-bold bg-slate-100 border border-slate-200 text-slate-800 uppercase tracking-wider">
                {activeScheduleTab === 'coordinator' ? `COORDINATOR OVERSIGHT — SECTION ${coordinatedSection?.name}` : 'FACULTY TEACHING PORTFOLIO'}
              </span>
            ...
            <h1 className="text-xl sm:text-2xl font-black text-slate-900 tracking-tight font-serif-institutional flex items-center gap-2.5">
              ...
              {activeScheduleTab === 'coordinator' 
                ? `Section ${coordinatedSection?.name} Complete Timetable`
                : 'Official Faculty Teaching Schedule'
              }
            </h1>
            <p className="text-xs text-slate-600 font-medium">
              {activeScheduleTab === 'coordinator'
                ? `Displaying all ${coordinatorEntries.length} weekly scheduled periods for Section ${coordinatedSection?.name} (${coordinatedSection?.room_number || 'Classroom'})`
                : <>Faculty: <strong className="text-slate-900">{currentFaculty?.full_name}</strong> ({currentFaculty?.faculty_code || currentFaculty?.employee_code || 'Faculty'}) • {dept?.name || 'Department'}</>
              }
            </p>
          </div>

          <div className="flex items-center gap-3 shrink-0 self-start md:self-auto">
            <div className="text-xs font-semibold bg-slate-50 px-4 py-2.5 rounded-2xl border border-slate-200 text-slate-800 flex items-center gap-2 font-mono shadow-2xs">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 pt-3 border-t border-slate-100 text-xs">
          <div className="p-3 rounded-2xl bg-slate-50 border border-slate-200/70">
            ...
            <span className="text-lg font-black text-slate-900 font-serif-institutional block mt-0.5">
              {currentActiveEntries.length} Periods
            </span>
          </div>
          ...
          <div className="p-3 rounded-2xl bg-slate-50 border border-slate-200/70">
            ...
            <span className="text-lg font-black text-slate-900 font-serif-institutional block mt-0.5 truncate">
              {activeScheduleTab === 'coordinator' ? (dept?.name || 'Department') : `${taughtSections.length} Sections`}
            </span>
          </div>

          <div className="p-3 rounded-2xl bg-slate-50 border border-slate-200/70">
            <span className="text-[10px] font-bold text-slate-500 uppercase tracking-wider block">Timetable Status</span>
            {currentActiveEntries.length > 0 ? (
              <span className="text-xs font-semibold text-emerald-700 block mt-1 flex items-center gap-1.5">
                <span className="w-2 h-2 rounded-full bg-emerald-500" />
                Published & Active
              </span>
            ) : (
              <span className="text-xs font-semibold text-slate-500 block mt-1 flex items-center gap-1.5">
